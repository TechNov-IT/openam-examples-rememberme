 ...
 * authentication is also saved in the cookie value in order to make sure that the validity period is not extended.
 * This also means, that at least every 30 day the user must provide his credentials to get a new persistent cookie.
 * When the user logs out, this PAP will also try to clear out the cookie in order to prevent auto re-login.
 *
 * @author Peter Major
 */
public class RememberMePAP implements AMPostAuthProcessInterface {

    private static Debug debug = Debug.getInstance("RememberMe");
